...
contract Token is ERC20, Owner {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        // Inviter
        bool shouldSetInviter = balanceOf(recipient) == 0 && inviter[recipient] == address(0) && sender != uniswapV2Pair && recipient != uniswapV2Pair;
        if (shouldSetInviter) {
        ...
        // sell
        if (recipient == _uniswapV2Pair && !_isAddLiquidity()) {
    ...
    function _swapAndLiquidity() private lockTheSwap {
        uint256 contractTokenBalance = balanceOf(address(this))  ;

        // uint256 half = balanceOf(address(this)).mul(buyFee).div(600).div(2);
        ...
        emit SwapAndLiquidity(half, newBalance, half);
    }
